...
export function ThemeEditor(props) {
  ...
  const onNewScanResult = async (decodedText, decodedResult) => {
    ...
    try {
      ...
      setSearchData(data || []); // Optional chaining in case data is undefined
    } catch (error) {
          ...
          <DrawerBody>
            <>
              {showSearch ? (
                ...
                <Flex direction={"column"} gap={4} marginTop={10} backgroundColor={""} shadow={"medium"}>
                  <Button variant="outline" onClick={() => { setShowSearch(true) }}>Search Again</Button>

                  {searchData.length > 0 ? (
                    searchData.map((profile) => {
                      return (
                        <Box borderColor={"black"} borderWidth={1} borderRadius={12} padding={5}>
                          <Flex direction={"column"} gap={4}>
                            <Flex direction={"row"} gap={4} alignItems={"center"}>
                              <Avatar
                                ...
                                h="40px"
                              />
                              <Text fontSize="xl" as="b">{profile.data.name}</Text>
                            </Flex>

                            <Flex direction="row" style={{ flexWrap: "wrap" }} gap={4}>
                              <Text fontSize="md"> {profile.id}</Text>
                              <Text fontSize="md"> {profile.data.department}</Text>

                              <Flex gap="2">
                                <Text fontSize="md"> {profile.data.contact} </Text> /
                                <Text fontSize="md"> {profile.data.email} </Text>
                              </Flex>
                            </Flex>
