 ...
 * @param {object} allocationData - The allocation details (year, semester, subject, division).
 * @returns {Promise<void>}
 */
const allocateSubjectAPI = async (teacherId, allocationData) => {
  try {
    const teacherDocRef = doc(db, 'teachers', teacherId); // Teachers collection
    ...
    const allocatedSubjects = teacherDoc.data().allocatedSubjects || [];

    // Check if the subject is already allocated
    const isAlreadyAllocated = allocatedSubjects.some(
      (subject) =>
        subject.subject === allocationData.subject &&
        subject.division === allocationData.division &&
        subject.semester === allocationData.semester &&
        subject.year === allocationData.year
    );

    if (isAlreadyAllocated) {
      throw new Error('Subject is already allocated');
    }

    // Add the new subject to allocatedSubjects
    ...
    // Update Firestore
    await updateDoc(teacherDocRef, { allocatedSubjects: updatedSubjects });

    console.log('Subject allocated successfully');
  } catch (error) {
    console.error('Error in allocating subject:', error.message);
    throw error;
  }
};

/**
 * Revoke a subject from a teacher.
 * @param {string} teacherId - The ID of the teacher.
 * @param {object} revocationData - The revocation details (subject, division, semester, year).
 * @returns {Promise<void>}
 */
const revokeSubjectAPI = async (teacherId, revocationData) => {
  try {
    const teacherDocRef = doc(db, 'teachers', teacherId); // Teachers collection
    const teacherDoc = await getDoc(teacherDocRef);
    ...
    // Filter out the subject to be revoked
    const updatedSubjects = allocatedSubjects.filter(
      (subject) =>
        !(
          subject.subject === revocationData.subject &&
          subject.division === revocationData.division &&
          subject.semester === revocationData.semester &&
          subject.year === revocationData.year
        )
    );
    ...
    console.log('Subject revoked successfully');
  } catch (error) {
    console.error('Error in revoking subject:', error.message);
    throw error;
  }
};
...
module.exports = {
  ...
  firebaseElectiveDislocate,getStatCounts,revokeSubjectAPI,allocateSubjectAPI
};